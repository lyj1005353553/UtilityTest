...
public class RandomUtils {
     ...
     * 获取固定长度的随机字符串，它是大写字母、小写字母和数字的混合体。
     * @param length  length
     * @return  RandomUtils
     */
     ...
     * 得到一个固定长度的随机字符串，它是在sourceChar中混合的chars。
     ...
     * @return   get a fixed-length random string, its a mixture of chars in sourceChar
     */
    public static String getRandomStr(String[] sourceChar, int length) {
        if (sourceChar == null || sourceChar.length == 0 || length < 0) {
            return null;
        }
        ...
        Random random = new Random();
        for (int i = 0; i < length; i++) {
            str.append(sourceChar[random.nextInt(sourceChar.length)]).append(",");
        }
     ...
     * @return   get a fixed-length random string, its a mixture of chars in sourceChar
     */
    public static String getRandomInt(Integer[] sourceChar, int length) {
        if (sourceChar == null || sourceChar.length == 0 || length < 0) {
            return null;
        }
